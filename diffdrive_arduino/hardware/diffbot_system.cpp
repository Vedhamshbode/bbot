  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  for (const hardware_interface::ComponentInfo & joint : info_.joints)
  {
    ...
    {
      RCLCPP_FATAL(
        rclcpp::get_logger("DiffDriveArduinoHardware"), "Joint '%s' has %zu command interfaces found. 1 expected.", joint.name.c_str(),
    ...
    {
      RCLCPP_FATAL(
        rclcpp::get_logger("DiffDriveArduinoHardware"), "Joint '%s' have %s command interfaces found. '%s' expected.", joint.name.c_str(),
    ...
    {
      RCLCPP_FATAL(
        rclcpp::get_logger("DiffDriveArduinoHardware"), "Joint '%s' has %zu state interface. 2 expected.", joint.name.c_str(),
    ...
    {
      RCLCPP_FATAL(
        rclcpp::get_logger("DiffDriveArduinoHardware"), "Joint '%s' have '%s' as first state interface. '%s' expected.", joint.name.c_str(),
    ...
    {
      RCLCPP_FATAL(
        rclcpp::get_logger("DiffDriveArduinoHardware"), "Joint '%s' have '%s' as second state interface. '%s' expected.", joint.name.c_str(),
...
hardware_interface::CallbackReturn DiffDriveArduinoHardware::on_activate(const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  if (!comms_.connected())
  {
    RCLCPP_INFO(rclcpp::get_logger("DiffDriveArduinoHardware"), "not_connected");
    return hardware_interface::CallbackReturn::ERROR;
  }
  if (cfg_.pid_p > 0)
  {
      RCLCPP_INFO(rclcpp::get_logger("DiffDriveArduinoHardware"), "PID's Setting");
...
hardware_interface::return_type DiffDriveArduinoHardware::read(const rclcpp::Time & /*time*/, const rclcpp::Duration & period)
{
  ...
  // RCLCPP_INFO(rclcpp::get_logger("DiffDriveArduinoHardware"), wheel_l_.pos.itos(), wheel_l_.vel.itos(), wheel_r_.pos.itos(), wheel_r_.vel.itos());
